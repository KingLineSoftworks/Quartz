#====================================================================
# Compile the Jolt Physics library manually because its Jolt.cmake really sucks
#
...
function(add_jolt_library JOLT_CUSTOM_LIBRARY_NAME)
    ...
    # Create Jolt lib
    add_library(${JOLT_CUSTOM_LIBRARY_NAME} ${JOLT_PHYSICS_SRC_FILES})
    ...
    if (BUILD_SHARED_LIBS)
        ...
        target_compile_definitions(${JOLT_CUSTOM_LIBRARY_NAME} PUBLIC JPH_SHARED_LIBRARY)

        # Private define to instruct the library to export symbols for shared linking
        target_compile_definitions(${JOLT_CUSTOM_LIBRARY_NAME} PRIVATE JPH_BUILD_SHARED_LIBRARY)
    endif()
    ...
    function(EMIT_X86_INSTRUCTION_SET_DEFINITIONS)
        ...
        if (USE_AVX2)
            target_compile_definitions(${JOLT_CUSTOM_LIBRARY_NAME} PUBLIC JPH_USE_AVX2)
        endif()
        if (USE_AVX)
            target_compile_definitions(${JOLT_CUSTOM_LIBRARY_NAME} PUBLIC JPH_USE_AVX)
        endif()
        if (USE_SSE4_1)
            target_compile_definitions(${JOLT_CUSTOM_LIBRARY_NAME} PUBLIC JPH_USE_SSE4_1)
        endif()
        if (USE_SSE4_2)
            target_compile_definitions(${JOLT_CUSTOM_LIBRARY_NAME} PUBLIC JPH_USE_SSE4_2)
        endif()
        if (USE_LZCNT)
            target_compile_definitions(${JOLT_CUSTOM_LIBRARY_NAME} PUBLIC JPH_USE_LZCNT)
        endif()
        if (USE_TZCNT)
            target_compile_definitions(${JOLT_CUSTOM_LIBRARY_NAME} PUBLIC JPH_USE_TZCNT)
        endif()
        if (USE_F16C)
            target_compile_definitions(${JOLT_CUSTOM_LIBRARY_NAME} PUBLIC JPH_USE_F16C)
        endif()
        if (USE_FMADD AND NOT CROSS_PLATFORM_DETERMINISTIC)
            target_compile_definitions(${JOLT_CUSTOM_LIBRARY_NAME} PUBLIC JPH_USE_FMADD)
        endif()
    endfunction()
    ...
    if ("${CMAKE_CXX_COMPILER_ID}" STREQUAL "MSVC")
        if ("${CMAKE_VS_PLATFORM_NAME}" STREQUAL "x86" OR "${CMAKE_VS_PLATFORM_NAME}" STREQUAL "x64")
            if (USE_AVX512)
                target_compile_options(${JOLT_CUSTOM_LIBRARY_NAME} PUBLIC /arch:AVX512)
            elseif (USE_AVX2)
                target_compile_options(${JOLT_CUSTOM_LIBRARY_NAME} PUBLIC /arch:AVX2)
            elseif (USE_AVX)
                target_compile_options(${JOLT_CUSTOM_LIBRARY_NAME} PUBLIC /arch:AVX)
            endif()
        ...
        elseif ("${CMAKE_SYSTEM_PROCESSOR}" STREQUAL "x86_64" OR "${CMAKE_SYSTEM_PROCESSOR}" STREQUAL "AMD64" OR "${CMAKE_SYSTEM_PROCESSOR}" STREQUAL "x86" OR "${CMAKE_SYSTEM_PROCESSOR}" STREQUAL "i386")
            ...
            if (USE_AVX512)
                target_compile_options(${JOLT_CUSTOM_LIBRARY_NAME} PUBLIC -mavx512f -mavx512vl -mavx512dq -mavx2 -mbmi -mpopcnt -mlzcnt -mf16c)
            elseif (USE_AVX2)
                target_compile_options(${JOLT_CUSTOM_LIBRARY_NAME} PUBLIC -mavx2 -mbmi -mpopcnt -mlzcnt -mf16c)
            elseif (USE_AVX)
                target_compile_options(${JOLT_CUSTOM_LIBRARY_NAME} PUBLIC -mavx -mpopcnt)
            elseif (USE_SSE4_2)
                target_compile_options(${JOLT_CUSTOM_LIBRARY_NAME} PUBLIC -msse4.2 -mpopcnt)
            elseif (USE_SSE4_1)
                target_compile_options(${JOLT_CUSTOM_LIBRARY_NAME} PUBLIC -msse4.1)
            else()
                target_compile_options(${JOLT_CUSTOM_LIBRARY_NAME} PUBLIC -msse2)
            endif()
            if (USE_LZCNT)
                target_compile_options(${JOLT_CUSTOM_LIBRARY_NAME} PUBLIC -mlzcnt)
            endif()
            if (USE_TZCNT)
                target_compile_options(${JOLT_CUSTOM_LIBRARY_NAME} PUBLIC -mbmi)
            endif()
            if (USE_F16C)
                target_compile_options(${JOLT_CUSTOM_LIBRARY_NAME} PUBLIC -mf16c)
            endif()
            if (USE_FMADD AND NOT CROSS_PLATFORM_DETERMINISTIC)
                target_compile_options(${JOLT_CUSTOM_LIBRARY_NAME} PUBLIC -mfma)
            endif()

            if (NOT MSVC)
                target_compile_options(${JOLT_CUSTOM_LIBRARY_NAME} PUBLIC -mfpmath=sse)
            endif()
